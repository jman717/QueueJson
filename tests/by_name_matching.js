...
let qJson = new queue({
    ...
    stats: true,
    debug: true
}).init({ input_data: sample_data_by_name, matching: ['test', 'last'] })

try {
    qJson.process({}).then((success) => {
        qJson.logMsg(`by_name_matching success: (${JSON.stringify(success)})`, { "type": "success" })
    }, (error) => {
        qJson.logMsg(`by_name_matching errors: (${JSON.stringify(error)})`, { "type": "error" })
    })
} catch (e) {
